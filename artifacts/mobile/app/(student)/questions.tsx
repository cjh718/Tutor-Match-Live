...
export default function StudentQuestionsScreen() {
  ...
  const { data: questions, isLoading, refetch, isRefetching } = useGetQuestions(
    ...
    { query: { enabled: !!user?.userId, queryKey: getGetQuestionsQueryKey({ studentId: user?.userId }) } }
  );
  ...
  if (isLoading) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, padding: 20 }]}>
        {[1, 2, 3].map(i => <Skeleton key={i} height={88} style={{ marginBottom: 12 }} />)}
      </View>
    );
  }
          ...
          <Pressable onPress={() => router.push(`/question/${q.questionId}`)}>
            <Card style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={[styles.title, { color: colors.foreground }]} numberOfLines={1}>{q.title}</Text>
                <Badge label={q.status} variant={statusVariant(q.status)} />
              </View>
              <Text style={[styles.subject, { color: colors.mutedForeground }]}>{q.subject}</Text>
