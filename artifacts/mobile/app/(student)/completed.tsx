...
export default function StudentCompletedScreen() {
  ...
  const queryParams = { studentId: user?.userId, status: "Completed" as const };
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useGetSessions(queryParams, {
    query: {
      enabled: !!user?.userId,
      queryKey: getGetSessionsQueryKey(queryParams),
    },
  });
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            description="Completed tutoring sessions will appear here."
          />
        }
        renderItem={({ item: s }) => (
