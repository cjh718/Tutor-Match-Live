...
export default function StudentSessionsScreen() {
  ...
  const { data: sessions, isLoading, refetch, isRefetching } = useGetSessions(
    ...
    { query: { enabled: !!user?.userId, queryKey: getGetSessionsQueryKey({ studentId: user?.userId }) } }
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState icon="calendar" title="No sessions yet" description="Accept a tutor bid to schedule your first session." />
        }
        renderItem={({ item: s }) => (
