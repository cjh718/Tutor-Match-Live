...
export default function StudentDashboardScreen() {
  ...
  const { data: dashboard, isLoading, refetch, isRefetching } = useGetStudentDashboard(user?.userId || 0, {
    query: {
      enabled: !!user?.userId,
  ...
  return (
    ...
    >
      <Text style={[styles.greeting, { color: colors.foreground }]}>Hello, {user?.name}</Text>
      
      <View style={styles.statsGrid}>
        <Card style={styles.statCard}>
          <Feather name="help-circle" size={24} color={colors.primary} />
          <Text style={[styles.statValue, { color: colors.foreground }]}>{dashboard?.openQuestions || 0}</Text>
          <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>Open Questions</Text>
        </Card>
        <Card style={styles.statCard}>
          <Feather name="clock" size={24} color={colors.accent} />
          <Text style={[styles.statValue, { color: colors.foreground }]}>{dashboard?.pendingBids || 0}</Text>
          <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>Pending Bids</Text>
        </Card>
        <Card style={styles.statCard}>
      ...
      <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Recent Questions</Text>
